...
public class ReviewAnalysis {
	...
	public static double scoreReview(String review, String[] words, double[] scores, int[] wordCount, int numberOfWordsSoFar) {
		...
		Scanner word = new Scanner(review);
		while(word.hasNext()) {
			int wordIndex = indexOfWordInArray(words, word.next(), numberOfWordsSoFar);
			score += scores[wordIndex]/wordCount[wordIndex];
			numOfWords ++;
		}
		
		word.close();
		
		return score/numOfWords;
	}
	...
	public static int indexOfBestWord(double[] scores, int[] counts, int countAbove, int numberOfWordsSoFar) {
		...
		for(int i = 0; i < numberOfWordsSoFar; i++) {
			if(counts[i] > countAbove && scores[i] > bestScore) {
				bestScore = scores[i];
				index = i;
			}
		}
	...
	public static int processWords(Scanner s, int lineScore, String[] words, double[] word_score, int[] word_count, int numberOfWordsSoFar) {
		while(s.hasNext()) {
			String word = s.next();
			int index = indexOfWordInArray(words, word, numberOfWordsSoFar);
			if(index == -1) {//the token is not in words
				words[numberOfWordsSoFar] = word;
				index = numberOfWordsSoFar;
				numberOfWordsSoFar++;
			}
			
			word_score[index] += lineScore;
			word_count[index] += 1;
		}
	...
	public static String[] convertFileToStringArray(String filename) {
		...
		try {
			...
			Scanner fileForStrArr = new Scanner(myFile);
			String[] fileStrArr = new String[numOfLines];
			int i = 0;
			while(fileForStrArr.hasNextLine()) {
				fileStrArr[i] = fileForStrArr.nextLine().toLowerCase();
				i++;
			}
			fileForStrArr.close();
